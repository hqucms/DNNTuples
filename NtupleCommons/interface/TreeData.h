...
namespace deepntuples {
  ...
  public:
    TreeMultiVar(std::string name_) : AbstractTreeVar(name_) {}
    //  void fill() {}
  ...
  public:
    ...
    void add(std::string name, const Type defaultValue) {
      ...
      if (data.find(fullname) != data.end()) {
        if (isRead)
          return;  // ReadMode: the TreeVar has already been created; just return and call book() to set branch address later
    ...
    void addMulti(std::string name) {
      ...
      if (data.find(fullname) != data.end()) {
        if (isRead)
          return;  // ReadMode: the TreeVar has already been created; just return and call book() to set branch address later
        throw std::invalid_argument("[TreeData::addMulti] Variable w/ the same name has already been added: " +
                                    fullname);
      }
    ...
    void fill(std::string name, const FillType var, int mantissaBits = FloatPrecision::HIGH) {
      auto fullname = fullName(name);
      try {
        auto *tv = dynamic_cast<TreeVar<Type> *>(data.at(fullname));
        if (!tv)
          throw std::invalid_argument("[TreeData::fill] Filling a different type than registered: " + fullname);
        tv->fill((mantissaBits > 0 ? MiniFloatConverter::reduceMantissaToNbitsRounding(var, mantissaBits) : var));
      } catch (const std::out_of_range &e) {
        throw std::invalid_argument("[TreeData::fill] Variable " + fullname + " is not registered!");
      }
    }
    template <typename Type, typename FillType>
    void fillMulti(std::string name, const FillType var, int mantissaBits = FloatPrecision::HIGH) {
      auto fullname = fullName(name);
      try {
        auto *tv = dynamic_cast<TreeMultiVar<Type> *>(data.at(fullname));
        if (!tv)
          throw std::invalid_argument("[TreeData::fillMulti] Filling a different type than registered: " + fullname);
        tv->fill((mantissaBits > 0 ? MiniFloatConverter::reduceMantissaToNbitsRounding(var, mantissaBits) : var));
      } catch (const std::out_of_range &e) {
    ...
    template <typename Type>
    void set(std::string name, const Type var) {
      fill<Type, Type>(name, var);
    }
    template <typename Type>
    void setMulti(std::string name, const std::vector<Type> &vec) {
      ...
      try {
        auto *tv = dynamic_cast<TreeMultiVar<Type> *>(data.at(fullname));
        if (!tv)
          throw std::invalid_argument("[TreeData::setMulti] Filling a different type than registered: " + fullname);
        tv->set(vec);
      } catch (const std::out_of_range &e) {
    ...
    Type get(std::string name) const {
      ...
      try {
        auto *tv = dynamic_cast<TreeVar<Type> *>(data.at(fullname));
        if (!tv)
          throw std::invalid_argument("[TreeData::get] Wrong type: " + fullname);
        return tv->get();
      } catch (const std::out_of_range &e) {
    ...
    const std::vector<Type> &getMulti(std::string name) const {
      ...
      try {
        auto *tv = dynamic_cast<TreeMultiVar<Type> *>(data.at(fullname));
        if (!tv)
          throw std::invalid_argument("[TreeData::getMulti] Wrong type: " + fullname);
        return tv->get();
      } catch (const std::out_of_range &e) {
  ...
  protected:
    std::string fullName(std::string name) const { return prefix == "" ? name : (prefix + "_" + name); }

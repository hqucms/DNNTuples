...
from RecoJets.Configuration.RecoPFJets_cff import ak4PFJets, ak8PFJetsCHSSoftDrop, ak8PFJetsPuppiSoftDropMass, ak8PFJetsCHSPruned, ak8PFJetsCHSTrimmed, ak8PFJetsCHSFiltered, ak4PFJetsCHS, ca15PFJetsCHSMassDropFiltered, ak8PFJetsCHSConstituents
from RecoJets.JetProducers.ak8PFJetsCHS_groomingValueMaps_cfi import ak8PFJetsCHSPrunedMass, ak8PFJetsCHSTrimmedMass, ak8PFJetsCHSFilteredMass
...
def jetToolbox( proc, jetType, jetSequence, outputFile,
    ...
    algorithm = ''
    size = ''
    for TYPE, tmpAlgo in supportedJetAlgos.items():
...
#                                        'pfMassIndependentDeepDoubleBvLJetTags:probQCD', ### to check doubleB not running
...
#                                        'pfMassIndependentDeepDoubleCvBJetTags:probHcc',
    ...
    if not subjetBTagDiscriminators:
        ...
        if 'Puppi' in PUMethod:
            ...
            _addProcessAndTask( proc, mod["PFJets"],
                                ak4PFJetsPuppi.clone( src = cms.InputTag( srcForPFJets ),
                                                        applyWeight = False,  ### because source is already puppi
                                                        doAreaFastjet = True,
                                                        rParam = jetSize,
                                                        jetAlgorithm = algorithm ) )
    ...
    if addSoftDrop or addSoftDropSubjets:
        ...
        _addProcessAndTask( proc, mod["SoftDropMass"],
            ak8PFJetsPuppiSoftDropMass.clone( src = cms.InputTag( mod["PFJetsOrUpdate"] ),
    ...
    #################################################################################
    ####### Adding CMSTopTagger  (NOTE: it has not been validated or tested since 2017)
    if addCMSTopTagger :

        print('|======> CMSTopTagger has not been validated or tested since 2017. Use it on your own risk.')
    ...
    if addNsub:
        ...
        _addProcessAndTask( proc, mod["Njettiness"],
                            Njettiness.clone( src = cms.InputTag( mod["PFJetsOrUpdate"] ),
                                                srcWeights = cms.InputTag( (mod['puppi'] if 'Puppi' in PUMethod else '' )   ),
    ...
    if addEnergyCorrFunc:
        ...
        _addProcessAndTask(proc, mod["PFJetsSoftDropValueMap"],
                            cms.EDProducer("RecoJetToPatJetDeltaRValueMapProducer",
                                            ...
                                            distMax = cms.double(jetSize),
                                            values = cms.vstring(['userFloat("'+ecfSrc+'")' for ecfSrc in ecfSrcs]),
    ...
    if dataTier.startswith('nanoAOD'):
        ...
        for varName, varDef in jetVariables.items(): setattr( getattr( proc, mod['jetToolboxJetTable'] ).variables, varName.replace(":","_"), varDef )
        if runOnMC:
        ...
        if addPrunedSubjets or addSoftDropSubjets:
            ...
            for varName, varDef in subjetVariables.items(): setattr( getattr( proc, mod['jetToolboxSubjetTable']).variables, varName.replace(":","_"), varDef )
